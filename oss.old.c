...
static int request_deadlock_check(struct request_message *r,
  ...
  int i, avail[RMAX], need[RMAX], safe[SMAX];
  ...
  while(i != clen){
    ...
    for(i = 0; i < clen; i++){
      ...
      if(safe[i] == 0){  //if user process is not safe
        ...
        if(r[m->rd] >= m->val){ //if we have enough

          r[m->rd] += m->val; //return request resource to current resource available
...
static int unblock_requests(struct request_message * rmsg, const int nreq){
  ...
  for(i=0; i < bmsg_len; i++){
    ...
    if( (ptr->res[m->rd].val >= m->val) &&
        (request_deadlock_check(rmsg, nreq, &bmsg[i], bmsg_len - i) == 0) ){
  ...
  for(i=0; i < nreq; i++){
    ...
    if(m->op == REQUEST){
      ...
      if(nreq > 1){
        ...
        if(request_deadlock_check(&rmsg[i], (nreq - i), bmsg, bmsg_len)){
      ...
      ostat.releases++;
      printf("OSS has acknowledged Process P%d releasing R%d at time %ld:%ld\n",
        usr->id, m->rd, ptr->shared_clock.tv_sec, ptr->shared_clock.tv_usec);
